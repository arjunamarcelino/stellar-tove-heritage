...
import { emailSchema, registrationResponseSchema } from '@/lib/webauthn/schemas';
import { setAuthTokenCookies } from '@/lib/cookies';

// Issues the WebAuthn registration challenge. Email is normalized via the shared schema so it
// binds identically here and in finish (plan A4).
...
// Verifies the attestation, provisions the wallet, and — on success — sets httpOnly auth cookies
// then RETURNS (no redirect), so the client can show the transient success screen before
...
export async function finishPasskeyRegistrationAction(
  ...
  // Validate the attestation shape (defense-in-depth), but forward the ORIGINAL object so no fields
  // are stripped before the backend's cryptographic verification.
