...
// The quote validity countdown (TOV-176 / FR-06.03) — an isolated ticking LEAF so only it re-renders each
// second, never the whole confirmation. Mirrors RfqCountdown. Two facets that must never fork:
...
// "Expired" is derived client-side from validUntil vs now — the stored status may still read 'open' (no sweeper).
...
export default function QuoteCountdown({ validUntil }: { validUntil: string }) {
  const { days, hours, minutes, seconds, expired, pending } = useCountdown(validUntil);
  // target is constant, so the Intl.format (priciest per-tick call) is memoized off the ticking path.
  const spoken = useMemo(
    () => `Valid until ${SPOKEN_DATE_FMT.format(new Date(validUntil))}`,
    [validUntil],
  );
  ...
  return (
    <div role="timer" className="mt-2">
      ...
      <div aria-hidden="true" className="flex items-center gap-3">
        <span className="text-xs uppercase tracking-wide text-charcoal/50">Valid for</span>
