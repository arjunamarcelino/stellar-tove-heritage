...
/**
 * Extract the WebAuthn challenge the client signed, from clientDataJSON. Throws on
 * malformed input (the caller maps that to AUTH_PASSKEY_VERIFICATION_FAILED, not a 500).
 */
export function extractClientChallenge(response: RegistrationResponseJSON): string {
